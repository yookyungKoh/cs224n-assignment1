...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    # tokens: {'the':0, 'and':1, ...}
    # wordVectors: [N x m], N:#(words), m:embed_dim
    # sentence: [T x m] for T:#(words in the sentence)
    
    word2vec=[]
    for i in range(len(sentence)):
        word2vec.append(tokens[sentence[i]])
    
    sentVector = np.mean(word2vec)
...
def getRegularizationValues():
    ...
    values = LogisticRegression('l2', C=[-0.01, -0.1, 1.0, 5.0, 10.0, 100.0])
...
def chooseBestModel(results):
    ...
    bestResult = max(set(results), key=results('test'))
